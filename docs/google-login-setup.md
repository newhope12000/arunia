...
확인일: 2026-09-22. 서버가 Google 인증을 처리하는 웹 로그인 기준입니다. 현재 공개 배포는 기존 사이트를 `/`에, 새 읽기 전용 개편안을 `/preview/`에 제공합니다. 개편안에는 로그인 화면만 제공하며, 실제 로그인에는 서버·DB·OAuth 설정이 모두 필요합니다.
...
기존 프로젝트의 클라이언트로는 실제 로그인·새로고침 후 세션 유지·로그아웃을 확인했습니다. **새 전용 클라이언트로의 최종 사용자 로그인 확인은 대기 중**이며, 이전 검증 결과를 새 클라이언트의 성공으로 간주하지 않습니다. Vercel의 로그인 서버와 원격 DB는 연결하지 않았습니다. 공개 `/preview/`와 정적 공유 링크에서는 Google 로그인·상담 접수·결제를 받지 않습니다.
...
환경 변수를 저장한 뒤 로컬 서버를 다시 시작합니다. 향후 서버 기능을 공개할 때는 Vercel 서버 환경에 동일한 이름으로 등록하고, 배포 환경에 맞게 `APP_ORIGIN`을 설정해야 합니다. OAuth 값은 서버용으로만 사용하며 `VITE_` 접두사를 붙이지 않습니다. `.env`·다운로드한 인증 JSON·비밀키를 `public/` 또는 GitHub에 올리지 않습니다. [Google 자격증명 보관 안내](https://developers.google.com/identity/protocols/oauth2/web-server)

`npm run build:review`는 기존 정적 사이트를 루트에, 개편안을 `/preview/`에 배치합니다. 공개 개편안은 API·DB를 사용하지 않으며 Vercel의 `/api` 요청도 `ARUNIA_REVIEW_ONLY=true`로 차단합니다. 키를 추가하는 것만으로는 로그인·상담 접수·결제가 활성화되지 않습니다. 실제 계정의 로컬 검증은 기존과 동일하게 `npm run dev`로 전체 서버를 실행한 뒤 `http://localhost:4173/login`에서 진행합니다.

공개 `/preview/login`은 로그인 디자인 확인용입니다. 향후 이 경로에서 실제 로그인을 제공하려면 루트의 기존 사이트를 보존하는 빌드와 인증 후 복귀 경로를 함께 준비해야 합니다. Google에 등록된 서버 콜백은 `/api/auth/google/callback`이며, 정적 화면의 `/preview/` 경로를 이유로 콜백 앞에 임의로 접두사를 붙이지 않습니다. 후속 작업은 [Vercel 연결 체크리스트](vercel-google-login-checklist.md)를 참고합니다.

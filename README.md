...
`npm run build:review`가 세 경로를 함께 `review-dist/`에 생성합니다. 이번 시안의 코드·이미지는 `renewal/`, 생성기는 `scripts/build-renewal.mjs`, 이미지 프롬프트는 `renewal/image-prompts.json`에 있습니다. 한국 청년 사진은 AI 연출 예시, 경험 이야기는 가상 사례로 표시했습니다. 기존 콘텐츠 아카이브·라운드테이블·DAY ONE 클래스와 무료·베이직·프리미엄 구독을 기본으로 안내합니다. 실제 가입·결제·문의 제출 기능은 연결하지 않았으며, 4기 지원만 승인된 외부 페이지로 연결합니다. 현재 외부 지원 페이지는 ChatGPT 로그인을 요구할 수 있습니다.
